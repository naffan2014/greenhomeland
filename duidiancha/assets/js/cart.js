...
//商品数量加减一
function addCartNumYu(num,i) {
    var numObj = $("#commoditySelectNum"+i);
    var  priceonj=$("#Hiprice"+i);
    var selectNum = 0;
    if (numObj.val().length > 0) {
        selectNum = parseInt(numObj.val());
    }
    ...
    var totalAmount =parseInt(selectNum)*parseFloat(priceonj.val());
    $("#showprice" + i).text(parseFloat(totalAmount).toFixed(2));
    numObj.val(selectNum);
}
//初始化商品规格事件
...
function cartAddyu(obj, webpath, linktype, linkurl,id) {
    var articleId = parseInt($("#commodityArticleId"+id).val());
    var goodsId = parseInt($("#commodityGoodsId" + id).val());
    var selectNum = parseInt($("#commoditySelectNum" + id).val());
...
//添加进购物车
function cartAdds(obj, webpath, linktype, linkurl, ArticleId) {
